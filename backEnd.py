...
class clusterModel:
  ...
  def train(self, num_clusters, random_state = 0):
    if self.mode == "kmeans":
      ...
      kmeans_model = KMeans(n_clusters=num_clusters, random_state=random_state).fit(self.data)
...
class ourVisualizer:
  ...
  def scatter_plot(self, mode, colors):
    ...
    for s in self.indices_Dict[mode]:
      ...
      fig, ax = plt.subplots()
      ax.scatter(current_group[direction1], current_group[direction2], color=colors[0])
      ax.scatter(self.cur_group[direction1], self.cur_group[direction2], color=colors[2])
